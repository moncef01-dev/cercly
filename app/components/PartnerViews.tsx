...
const allowedMaterials: MaterialType[] = [
  "plastic",
  "carton",
  "battery",
  "printer_cartridge",
  ...
  function handleRequestCollection() {
    ...
    setTimeout(() => {
      ...
      addCollectionPoint({
        ...
        lat: 36.35 + Math.random() * 0.02,
        lng: 6.62 + Math.random() * 0.02,
      ...
      const totalPoints = reqMaterials.reduce(
        (sum, mat) =>
          sum +
          calcPoints(mat, weight / Math.max(reqMaterials.length, 1), bottles),
        0,
      );
      updatePartnerProfile({
        points: profile.points + totalPoints,
        totalRecycled: profile.totalRecycled + weight / 1000,
      });
  ...
  const impactItems = useMemo(
    () => [
      { icon: "🌳", label: "أشجار محمية", val: `${profile.treesProtected}` },
      {
        icon: "💧",
        label: "مياه موفرة",
        val: `${profile.waterSaved.toLocaleString()} ل`,
      },
      {
        icon: "☁️",
        label: "انبعاثات CO₂ المخفضة",
        val: `${Math.round(profile.co2Saved * 100)} كجم`,
      },
    ],
    [profile],
  );
        ...
        <div className="card">
          <div className="flex-between">
            ...
            <span className="text-sm" style={{ fontWeight: 600 }}>
              {profile.points} / {nextLevelPoints} نقطة
            </span>
          </div>
        ...
        <div className="kpi-grid">
          ...
          <div className="kpi-card">
            <div className="kpi-metric">
              {Math.round(profile.totalRecycled * 1000).toLocaleString()}
            </div>
            <div className="kpi-label">كجم مجمع</div>
          </div>
          <div className="kpi-card">
            <div className="kpi-metric">
              {Math.round(profile.co2Saved * 100)}
            </div>
            ...
            >
              <div className="form-row">
                ...
                <input
                  ...
                  placeholder="أدخل عنوان موقع الجمع"
                />
              </div>
              <div className="form-row">
                ...
                >
                  {materials
                    .filter((m) => allowedMaterials.includes(m.id))
                    .map((m) => (
                      <label
                        key={m.id}
                        style={{
                          ...
                          cursor: "pointer",
                          fontSize: "13px",
                          background: reqMaterials.includes(m.id)
                            ? "var(--light-green)"
                            : "#fff",
                          color: "var(--text-dark)",
                        }}
                      >
                        <input
                          type="checkbox"
                          checked={reqMaterials.includes(m.id)}
                          onChange={() => toggleReqMaterial(m.id)}
                        />{" "}
                        {m.icon} {m.name}
                      </label>
                    ))}
              ...
              <div style={{ display: "flex", gap: "12px" }}>
                ...
                <div className="form-row" style={{ flex: 1 }}>
                  ...
                  <input
                    ...
                    type="number"
                    value={reqBottles}
                    onChange={(e) => setReqBottles(e.target.value)}
                    placeholder="عدد القوارير"
                  />
                </div>
              </div>
              <div style={{ display: "flex", gap: "12px" }}>
              ...
              {reqMaterials.length > 0 && (
                ...
                >
                  <div
                    className="text-sm"
                    style={{ fontWeight: 700, color: "var(--primary-green)" }}
                  >
                    النقاط المتوقعة:
                  </div>
                  {reqMaterials.map((mat) => {
                    ...
                    const pts = calcPoints(
                      mat,
                      weight / Math.max(reqMaterials.length, 1),
                      bottles / Math.max(reqMaterials.length, 1),
                    );
                    const matName =
                      materials.find((m) => m.id === mat)?.name ?? mat;
                    return (
                      <div
                        key={mat}
                        className="text-sm"
                  ...
                  >
                    ...
                    <div className="btn-row">
                      <button
                        ...
                        onClick={() => handleConfirm(sch.id, sch.pointName)}
                      >
                      ...
                      <button
                        ...
                        onClick={() => handlePostpone(sch.id, sch.pointName)}
                      >
                        تأجيل
                      </button>
                    </div>
            ...
            fallback={
              <div
                style={{
                  ...
                  background:
                    "linear-gradient(90deg, #EBE8E0 25%, #F5F3EE 50%, #EBE8E0 75%)",
                  display: "flex",
                  alignItems: "center",
            ...
            {trucks.map((truck, i) => (
              <div className="order-row" key={truck.id}>
                <div
                  ...
                  style={{ background: "#FFFDF6", color: "var(--gold)" }}
                >
                  🏍️
                </div>
                <div className="order-info">
                ...
                <span
                  className={`badge ${truck.status === "loading" ? "badge-orange" : truck.status === "en_route" ? "badge-blue" : "badge-green"}`}
                >
        ...
        <div className="card">
          ...
          >
            {rewards.map((reward) => (
              <div
                className="kpi-card"
                style={{ padding: "16px 12px", gap: "6px" }}
                key={reward.id}
              >
                ...
                <button
                  ...
                  disabled={
                    profile.points < reward.pointsCost || isRedeeming !== null
                  }
            ...
            {pointsHistory.slice(0, 10).map((ph) => (
              <div className="order-row" key={ph.id}>
                <div
                  ...
                  style={{
                    background:
                      ph.type === "earned" ? "var(--light-green)" : "#FFFDF6",
                    color:
                      ph.type === "earned"
                        ? "var(--primary-green)"
                        : "var(--gold)",
  ...
  if (currentTab === "profile") {
    return (
      <>
        ...
        <div
          className="card"
          style={{ textAlign: "center", padding: "24px", alignItems: "center" }}
        >
          <div
            style={{
              width: "64px",
          ...
          <div style={{ marginTop: "6px" }}>
            <span className="badge badge-green">مستوى {level} — مشارك نشط</span>
          </div>
        </div>
        <div className="card">
          <div className="form-row">
